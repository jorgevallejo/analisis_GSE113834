...
```{r probes without Entrez code}
# Check if there are probes without Entrez code
no_entrez <- sum(IQR_noprobes$Entrez.Gene == "",
                 is.na(IQR_noprobes$Entrez.Gene),
                 IQR_noprobes$Entrez.Gene == "---")
# Nevertheless, I know that there are probes that map to
# two or more different EntrezIDs. I don't know how to
# handle those cases.
```
Sondas sin código Entrez: `r no_entrez`

```{r sondas para el mismo codigo Entrez}
# Order files (descendant) for entrezId, then for IQR
ordered_IQR <- IQR_noprobes[ order(IQR_noprobes$Entrez.Gene, 
                                   -IQR_noprobes$IQR), ]
# Make logical vector of duplicates
dup_entrez <- duplicated(ordered_IQR)
```
