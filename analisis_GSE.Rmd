...
```{r construct ExpressionSet object, eval=FALSE}
# Read the CEL files
library('oligo')
...
rawData <- read.celfiles(celFiles, phenoData = my.targets, pkgname="primeviewprobe")
```

```{r construct ExpressionSet with GEOquery}
library(GEOquery)
gse <- getGEO("GSE113834", GSEMatrix = TRUE, AnnotGPL = TRUE)
