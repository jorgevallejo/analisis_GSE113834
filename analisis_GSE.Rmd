...
A partir del diagrama de Venn, vemos que 223 genes están diferencialmente expresados sólo en las muestras control, y 627 sólo en las muestras ASD. Son estos dos grupos de genes los que en principio nos interesa explorar.
...
print(summary(res.selected2))

```

```{r venn diagram lista interes, fig.height=4.5, fig.width=4.5}
vennDiagram(res.selected2[, 1:3], cex=0.9)
title("Genes en común entre dos comparaciones\n Seleccionados con FDR <0.1",
      cex.main=0.9)
```
...
colsidecolors <- c("yellow", "orange", "red", "yellow", 
