...
Sondas filtradas por corresponder a más de un gen:`r format(length(multGenes), big.mark=" ")`

En un caso real, me sentiría más cómodo examinando cada caso en el que una de estas sondas, correspondientes a más de un gen, aparecieran en las listas de genes diferencialmente expresados ya que pueden explicarse por secuencias pertenecientes a familias génicas muy cercanas, o por pertenecer a transcritos que 'corran' a lo largo de varios genes. En este caso, sin embargo, debido a que se trata de un ejercicio de aprendizaje, y a la limitación de tiempo, me he limitado a eliminar de la lista aquellas sondas que pueden identificar más de un gen.
...
fit.main2 <- contrasts.fit(fit, cont.matrix2)
fit.main2 <- eBayes(fit.main2)
```

## Lists of differentially expressed gens
### Comparación 1 (CvsA.INPUT): Genes que cambian su expresión entre las condiciones "Control" y "ASD" detectados en muestras de ARN total (INPUT).
```{r topTab lists CvsA.INPUT}
topTab_CvsA.INPUT <- topTable(fit.main, number=nrow(fit.main), coef="CvsA.INPUT",
                              adjust="fdr")
head(topTab_CvsA.INPUT)

topTab_CvsA.INPUT2 <- topTable(fit.main2, number=nrow(fit.main2), coef="CvsA.INPUT",
                              adjust="fdr")
```

### Comparación 2 (CvsA.WASH): Genes que cambian su expresión entre las condiciones "Control" y "ASD" detectados en muestras de ARNm enriquecidas para colas poli-A cortas (WASH).
```{r topTab lists CvsA.WASH}
topTab_CvsA.WASH <- topTable(fit.main, number=nrow(fit.main), coef="CvsA.WASH",
                              adjust="fdr")
head(topTab_CvsA.WASH)

topTab_CvsA.WASH2 <- topTable(fit.main2, number=nrow(fit.main2), coef="CvsA.WASH",
                              adjust="fdr")
```

### Comparación 3 (CvsA.ELUTED): Genes que cambian su expresión entre las condiciones "Control" y "ASD" detectados en muestras de ARNm enriquecidas para colas poli-A largas (ELUTED).
```{r topTab lists CvsA.ELUTED}
topTab_CvsA.ELUTED <- topTable(fit.main, number=nrow(fit.main), coef="CvsA.ELUTED",
                              adjust="fdr")
head(topTab_CvsA.ELUTED)

topTab_CvsA.ELUTED2 <- topTable(fit.main2, number=nrow(fit.main2), coef="CvsA.ELUTED",
                              adjust="fdr")
```

### Comparación 4 (INT): Genes que cambian su expresión de forma significativa por interacción entre la condición de la muestra (CONTROL, ASD) y el material examinado (INPUT, WASH, ELUTED).
```{r topTab lists CvsA.INT}
topTab_INT <- topTable(fit.main, number=nrow(fit.main), coef="INT",
                              adjust="fdr")
head(topTab_INT)
```

Las tablas de este apartado son tan sólo muestras, y la primera columna contiene la ID de las sondas de Affymetrix, no códigos de genes. En el siguiente apartado llevaremos a cabo la anotación de los resultados para poder trabajar con los códigos de los genes en lugar de con las sondas.

# Anotación de los genes
A partir de los datos extraídos de GPL15207 (gpl_table) vamos a asociar el código de cada sonda al símbolo del gen correspondiente, el nombre del gen y el código de identificación en Entrez; para cada lista de genes diferencialmente expresados.
```{r anotar toptables}
...
anotopTab_CvsA.INPUT <- anottopTap(topTab_CvsA.INPUT)
anotopTab_CvsA.WASH <- anottopTap(topTab_CvsA.WASH)
anotopTab_CvsA.ELUTED <- anottopTap(topTab_CvsA.ELUTED)
anotopTab_INT <- anottopTap(topTab_INT)

anotopTab_CvsA.INPUT2 <- anottopTap(topTab_CvsA.INPUT2)
anotopTab_CvsA.WASH2 <- anottopTap(topTab_CvsA.WASH2)
anotopTab_CvsA.ELUTED2 <- anottopTap(topTab_CvsA.ELUTED2)
```

Muestra de la tabla CvsA.INPUT anotada:
```{r sample annotated CvsA.INPUT}
head(anotopTab_CvsA.INPUT)
```

# Visualización (volcanoplots)
```{r volcanoplots, fig.height=16, fig.width=5}
par(mfrow=c(4, 1))
...
```{r volcanoplots2, fig.height=12, fig.width=5}
par(mfrow=c(3, 1))

contrastes2 <- colnames(fit.main2$coefficients)

for (contrast in 1:length(contrastes2)){
  volcanoplot(fit.main2, 
              coef = contrastes2[contrast],
              highlight = 10, 
              names = eval(str2expression(
                paste0("anotopTab_", contrastes2[contrast], "2$Gene.Symbol"))),
              main=paste("Genes diferencialmente expresados", contrastes2[contrast],
                         sep = "\n"))
abline(v=c(-1,1))
}
```
# Comparaciones múltiples
¿Qué genes cambian simultáneamente en más de una comparación?
```{r decidetests}
# as many columns as comparisons and as many rows as genes.
res <- decideTests(fit.main, method="separate", adjust.method="fdr", p.value=0.2,
                   lfc = 0)
# This table can be annotated and saved as a csv file, for example

res2 <- decideTests(fit.main2, method="separate", adjust.method="fdr", p.value=0.2,
                   lfc = 0)
```
...
res.selected2 <- res[sum.res.rows2!=0, ]
print(summary(res2))
```
