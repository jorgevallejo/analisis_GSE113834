...
```{r}
graph_colours <- rainbow(length(rawData))
```
...
legend(x=11.9, y=0.31, legend=pData(rawData)$X.Abreviado, fill=graph_colours, ncol=2, bty="n",
       cex=0.5)
```
...
plotPCA3(exprs(rawData), labels = pData(rawData)$X.Abreviado, factor = pData(rawData)$X.Grupo, title="Datos brutos", scale = FALSE, size = 3, colores = rainbow(6))
```
...
library(affyPLM)
# Generate PLMset
if(file.exists("intermediateResults/Pset.RData")){
  load("intermediateResults/Pset.RData")
  }else{
    Pset <- fitPLM(rawData)
    save(Pset, file="intermediateResults/Pset.RData")
  }

#pseudo image of the weights for the first array in the dataset
image(Pset, which=1, main=noquote(Pset@phenoData@data$X.Abreviado.[1]))
```
